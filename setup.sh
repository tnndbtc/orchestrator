...
do_show_usage() {
    ...
    printf '      Stage 1  generate_script        → DraftStory.json → StoryPrompt.json → Script.json\n'
    printf '               If world-engine is installed, validate-story-draft runs between\n'
    printf '               DraftStory and StoryPrompt; canon violations block the pipeline.\n'
    ...
    printf '      Stage 5  render_preview          → RenderOutput.json  (+ output.mp4 when\n'
    printf '                                          video binary is installed)\n'
    printf '\n'
