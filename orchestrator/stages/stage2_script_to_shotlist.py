"""Stage 2: Convert Script to ShotList via world-engine or deterministic stub."""
...
def _call_world_engine(project_id: str, run_id: str, registry: ArtifactRegistry) -> dict | None:
    """Try world-engine produce-shotlist; return ShotList dict on success, None otherwise."""
    script_path = registry.artifact_path(project_id, run_id, "Script")
...
def run(project_config: dict, run_id: str, registry: ArtifactRegistry) -> dict:
    """Derive ShotList from Script via world-engine or stub.

    Reads:  Script.json
    Writes: ShotList.json

    world-engine path: calls `world-engine produce-shotlist` for a canonical ShotList.
    Stub fallback: 2 shots per scene, duration derived from dialogue word count.
    """
    project_id = project_config["id"]
    script = registry.read_artifact(project_id, run_id, "Script")

    shotlist = _call_world_engine(project_id, run_id, registry)

    if shotlist is None:
        # Fallback: derive stub ShotList from Script.
        shots: list[dict] = []
